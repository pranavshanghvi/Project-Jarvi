...
/* The reader's own turns, set apart from the tutor's so a long thread stays readable as a
   conversation rather than a wall of cards. */
.asked{margin:1.4rem 0 .2rem;padding-left:.9rem;border-left:2px solid var(--line)}
.asked p{margin:0;color:var(--soft);font-size:.95rem}
.reply{margin:1.2rem 0 2rem}
...
.bar{height:4px;background:var(--surface2);border:1px solid var(--line);border-radius:2px;overflow:hidden}
...
<div class="wrap">
  ...
  <section class="view on" id="v-read">
    <div id="read-index">
      ...
      <p class="tap-hint">Open a section, then tap any sentence to ask about it.</p>
      <div id="toc"></div>
    </div>
  ...
  <section class="view" id="v-set">
    ...
    <div style="margin-top:1.6rem">
      ...
      <p class="dim" style="margin-top:.5rem;font-size:.78rem">
        With a signal you can ask it anything — who Lorentz was, why a chapter is named after
        him, physics past the end of this book. It is not limited to what has been written here.
        It runs on free models only — Groq, Mistral, OpenRouter, Cloudflare — and when those are
        exhausted it says so and saves the question rather than falling back to anything paid.
        Answers are kept on this device, so once asked they work offline too.
      </p>
      <input class="q" id="provkey" type="password" style="margin-top:.55rem;font-family:var(--mono);font-size:.8rem"
             placeholder="Groq key (only if the server has none)" spellcheck="false" autocapitalize="off"
             autocomplete="off">
      <p class="dim" style="margin-top:.4rem;font-size:.75rem" id="provsum"></p>
      ...
      <div class="setrow"><span>Is it reachable?<br><span class="dim" id="testsum">Not checked</span></span>
        <button class="ghost" id="testgo">Test</button></div>
      <div class="setrow"><span>Ready for a flight<br><span class="dim" id="flightsum"></span></span>
        <button class="ghost" id="flightgo">Prepare</button></div>
      <div id="flightprog" style="display:none;margin-top:.6rem">
        <div class="bar"><i id="flightbar"></i></div>
        <p class="dim" id="flightmsg" style="margin:.45rem 0 0;font-size:.8rem"></p>
      </div>
    </div>
...
const defaults = {learner:{defaultLevel:"plain",needsSupport:[]},readingPosition:1,concepts:{},
  queued:[],theme:"auto",endpoint:__ENDPOINT__,live:"always"};
let prof;
...
// Sized for a flight-prep run rather than a browsing session: 37 sections plus every
// follow-up in the pack is ~123 answers, and there is no reason to start evicting mid-run.
const LIVE_MAX = 600;
let live = {};
...
// Kept out of the profile object so it is never caught up in anything that gets exported,
// logged or synced. Its own key, read and written in one place, and it only ever leaves this
// device inside a request to the reader's own endpoint over https.
const KEYKEY = "jarvi.relativity.providerkey";
const provKey = () => { try { return (localStorage.getItem(KEYKEY) || "").trim(); } catch { return ""; } };
function setProvKey(v){
  try { v ? localStorage.setItem(KEYKEY, v.trim()) : localStorage.removeItem(KEYKEY); } catch {}
}
// Sent only over https, or to localhost while developing. A mistyped http address should not be
// handed a credential.
function keysFor(url){
  const k = provKey();
  if(!k) return undefined;
  const https = url.slice(0,6).toLowerCase() === "https:";
  const local = url.indexOf("//localhost") > 0 || url.indexOf("//127.0.0.1") > 0;
  return (https || local) ? {GROQ_API_KEY: k} : undefined;
}

const endpoint = () => (prof.endpoint || "").trim();
// "always" is the default, and the reason is a question like "who was Lorentz". The pack has an
// entry near it and the book has a chapter named after him, so a gate that fires only when
// nothing matches decides the question is already answered — and hands back a passage about the
// transformation that never mentions the man. Topically close is not the same as answered, and
// no rule reliably tells the difference. With a signal, the tutor gets asked. The written
// explainers still come back underneath it.
// Anything unrecognised — an older stored value, a hand-edited profile — reads as the default
// rather than silently behaving like a mode nobody chose.
const LIVE_VALUES = ["always","gaps","off"];
const liveMode = () => LIVE_VALUES.includes(prof.live) ? prof.live : "always";
// A relative endpoint only means something when the page itself came off a web server. Opened
// straight out of Files on a Mac, "/api/ask" resolves to file:///api/ask, and trying it produces
// a failure card on a perfectly good offline session. In that case there is no endpoint.
const isAbsolute = s => {
  const p = String(s).slice(0, 8).toLowerCase();
  return p.startsWith("http://") || p.startsWith("https://");
};
const onTheWeb = () => location.protocol === "http:" || location.protocol === "https:";

// More than one address is tried, in order, because a hostname written into a build is exactly
// the kind of thing that is wrong once and then wrong forever. The page's own origin goes last
// and is the safety net: whatever served this HTML demonstrably exists, so if it also serves the
// endpoint, that is the one address that cannot be a typo.
function endpoints(){
  const out = [];
  const ep = endpoint();
  if(ep && isAbsolute(ep)) out.push(ep);
  else if(ep && onTheWeb()) out.push(location.origin + (ep.charAt(0) === "/" ? ep : "/" + ep));
  if(onTheWeb()) out.push(location.origin + "/api/ask");
  return out.filter((v, i) => out.indexOf(v) === i);
}
function usableEndpoint(){ return endpoints()[0] || ""; }
function canLive(){ return endpoints().length > 0 && navigator.onLine && liveMode() !== "off"; }

// One request to one address. Distinguishes the two failures that look identical to a reader
// but are completely different to fix: the server answered and refused (a real reply, act on
// it), versus nothing was there at all (wrong address, try the next one).
// ── the conversation ──────────────────────────────────────────────────────
// A tutor that answers once and stops is a vending machine. What a reader actually does is push
// back — "I still do not follow", "that is not what I asked", "I think you are wrong and here is
// why" — and none of that means anything unless the previous turns go with the question.
//
// One thread at a time. Asking something new from the reader or the Ask box starts a fresh one,
// because carrying an unrelated conversation into a new question makes the answers worse.
let thread = null;   // {ctx:{line,section}, turns:[{q, a}]}

function threadHistory(){
  if(!thread) return [];
  const h = [];
  for(const t of thread.turns){
    h.push({role:"user", content: t.q});
    if(t.a) h.push({role:"assistant", content: answerAsText(t.a)});
  }
  return h;
}

// The model's own words back to it, in the shape it produced them. Reconstructed rather than
// stored raw so a cached answer and a live one behave identically on the next turn.
function answerAsText(a){
  const bits = [];
  if(a.answer)  bits.push("ANSWER: " + a.answer);
  if(a.analogy) bits.push("ANALOGY: " + a.analogy);
  if(a.breaks)  bits.push("BREAKS: " + a.breaks);
  if(a.means)   bits.push("MEANS: " + a.means);
  if(a.next)    bits.push("NEXT: " + a.next);
  return bits.join("\\n");
}

async function askOne(url, body){
  const keys = keysFor(url);
  if(keys) body = Object.assign({}, body, {providerKeys: keys});
  ...
  try {
    ...
    try {
      ...
      // fetch() rejects for DNS failure, a refused connection, a TLS problem, or a CORS block.
      // The browser deliberately will not say which; all of them mean nothing usable is there.
      const e = new Error(ctl.signal.aborted ? "Timed out" : "Could not reach it");
      e.unreachable = true; throw e;
    }
    const data = await res.json().catch(()=>({}));
    if(!res.ok){
      const e = new Error(data.message || ("The tutor returned " + res.status));
      e.declined = data.declined; e.kind = data.error || ("http_" + res.status);
      e.status = res.status;
      throw e;                       // it answered — this IS the outcome, not a reason to retry
    }
    return data;
...
async function askLive(q, ctx){
  ...
  for(const url of list){
    try {
      const data = await askOne(url, body);
      livePut(q, sec ? sec.id : null, data);
      return data;
    } catch(e) {
      if(!e.unreachable) throw e;    // a real reply. Asking somewhere else would be wrong.
      missed.push(url);
    }
  }
  const e = new Error(list.length > 1
    ? "None of the tutor addresses answered."
    : "Could not reach the tutor.");
  e.unreachable = true; e.tried = missed;
  throw e;
}
...
function renderLive(a, q){
  let h = '<div class="ans live"><div class="lbl live-tag">Live tutor</div>'
        + (q ? '<h3>'+esc(q)+'</h3>' : '')
        + '<div class="lvlbody">'+para(a.answer||"")+'</div>';
...
// The reply affordance. Typing a full sentence of frustration on a phone is work, so the three
// things a stuck reader most often means are one tap; anything more specific goes in the box.
const REPLIES = [
  ["I still don't follow",  "I still do not follow. Explain it a completely different way — a different analogy, or a concrete case with numbers."],
  ["Simpler",               "Simpler, please. Assume I know no physics at all."],
  ["Why is that true?",     "Why is that true? What is the reasoning behind it?"],
];

function renderThread(){
  if(!thread) return "";
  let h = "";
  if(thread.ctx && thread.ctx.line){
    const s = thread.ctx.section;
    h += '<div class="ans"><h3>'+(s && s.numeral ? "§"+esc(s.numeral)+" · " : "")+esc(s ? s.title : "This line")+'</h3>'
       + '<p class="quote" style="font-family:var(--serif);color:var(--soft);border-left:2px solid var(--accent);padding-left:.6rem;margin:.4rem 0 0">'
       + esc(thread.ctx.line)+'</p></div>';
  }
  for(const t of thread.turns){
    h += '<div class="asked"><div class="lbl">You asked</div><p>'+esc(t.q)+'</p></div>';
    h += t.a ? renderLive(t.a, null) : (t.err || THINKING);
  }
  if(thread.turns.length && thread.turns[thread.turns.length-1].a) h += replyBox();
  return h + (thread.below || "");
}

// Only offered when a reply can actually be sent. An input that silently does nothing offline is
// worse than no input.
function replyBox(){
  if(!canLive()) return '<p class="dim" style="margin:.4rem 0 1.4rem">'
    + (navigator.onLine ? "Switch the live tutor on in Settings to carry on this conversation."
                        : "Back on a signal you can carry on this conversation.") + '</p>';
  return '<div class="reply"><div class="lbl">Not convinced? Say so</div>'
    + '<div class="chips">' + REPLIES.map(r =>
        '<button class="chip" data-reply="'+esc(r[1])+'">'+esc(r[0])+'</button>').join("")
    + '</div><div class="askrow"><input id="replybox" placeholder="or tell it exactly what is wrong…" enterkeyhint="send">'
    + '<button class="iconbtn go" id="replysend">Send</button></div></div>';
}
...
function liveError(e, q){
  ...
  // A wrong address and an exhausted rotation feel identical from the sofa, so the address is
  // named when the request never landed. It is the one failure the reader can actually fix.
  const unreachable = !!e.unreachable;
  const tried = e.tried && e.tried.length ? e.tried : endpoints();
  return '<div class="miss"><strong>'
    + (exhausted ? "The free models are all out for now."
                 : esc(e.message || "Could not reach the tutor."))
    + '</strong>'
    + (unreachable ? '<p style="margin:.6rem 0 0">Nothing answered at:</p>'
        + '<p style="margin:.35rem 0 0;font-family:var(--mono);font-size:.75rem;word-break:break-all">'
        + tried.map(u=>"· "+esc(u)).join("<br>") + '</p>'
        + '<p style="margin:.6rem 0 0">Either the address is wrong, or that server has no tutor '
        + 'endpoint yet. Settings lets you change it.</p>' : '')
    + '<p style="margin:.6rem 0 0">Saved your question — try again later.</p>'
...
// A free tier can take twenty seconds, which is long enough to ask something else in the
// meantime. Without this counter the first answer lands last and silently replaces the second —
// the reader sees a reply to a question they have moved on from, with no clue why.
let askSeq = 0;
...
async function runLive(q, ctx, box, above, below){
  ...
  if(mine !== askSeq) return null;      // overtaken; the answer is cached either way
  box.innerHTML = above + card + below;
  window.scrollTo(0,0);
  return box.querySelector(".ans.live");
}

// Start a new conversation about a line or a question.
// The 'below' argument is the written material that goes under the conversation. It is part of the thread
// rather than passed per-turn, because it should stay put as the conversation grows.
function startThread(q, ctx, below){
  thread = {ctx: ctx || {}, turns: [], below: below || ""};
  return continueThread(q);
}

// Add a turn to the conversation that is already open. Everything said so far goes with it,
// which is the whole difference between a tutor and a search box.
async function continueThread(q){
  if(!thread) return;
  const mine = ++askSeq;
  const turn = {q, a: null, err: null};
  thread.turns.push(turn);
  const box = document.getElementById("answers");
  showView("ask");
  box.innerHTML = renderThread();
  const last = box.querySelector("#thinking");
  if(last) last.scrollIntoView({block:"center", behavior:"smooth"});

  const history = threadHistory().slice(0, -1);   // everything before this question
  try {
    turn.a = await askLive(q, {line: thread.ctx.line, section: thread.ctx.section, history});
  } catch(e) {
    queueQuestion(q);
    turn.err = liveError(e, q);
  }
...
// A one-tap answer to "why is it not working", because the three causes — wrong address, no
// endpoint there, no provider key — produce the same silence from the reader's side and need
// completely different fixes.
async function testEndpoints(){
  const out = document.getElementById("testsum");
  const list = endpoints();
  if(!list.length){ out.textContent = "No address to test"; return; }
  out.textContent = "Checking " + list.length + " address" + (list.length > 1 ? "es" : "") + "…";
  const lines = [];
  for(const url of list){
    const host = url.replace("https://","").replace("http://","").split("/")[0];
    try {
      await askOne(url, {question: "ping"});
      lines.push("✓ " + host + " — answering");
      break;                                   // first one that works is the one that gets used
    } catch(e) {
      lines.push(e.unreachable       ? "✗ " + host + " — nothing there"
        : e.kind === "no_free_provider" ? "✓ " + host + " — reachable, but no provider key set"
        : e.status === 404            ? "✗ " + host + " — reachable, but no tutor endpoint"
        : "✓ " + host + " — reachable (" + (e.message || e.kind) + ")");
      if(!e.unreachable) break;                // it answered; the rest will not be consulted
    }
  }
...
// The written pack is 47 entries against a thirty-thousand-word book, which is thin, and no
// amount of hand-writing closes that gap. What does close it is that the live tutor can answer
// anything and every answer it gives is kept on the device — so the fix is to ask it the
// predictable questions while there is still a signal.
//
// Predictable means: what is each section actually saying (the reader goes through the book in
// order, so all 37 will come up), every follow-up the pack already offers, and every
// misconception. Around 123 questions, which at the server's rate limit is roughly ten minutes
// of sitting in a departure lounge.
//
// It does not make the app omniscient offline. A question nobody anticipated still needs a
// signal. It moves the offline floor from "47 entries" to "every section explained properly,
// plus everything you asked before".
function flightQuestions(){
  const out = [];
  for(const s of C.sections)
    out.push({q: "What is this section actually saying, and why does it matter?", section: s});
  for(const e of P.entries)
    for(const n of ((e.edges && e.edges.next) || [])) out.push({q: n.q, section: null});
  for(const id in P.misconceptions)
    out.push({q: P.misconceptions[id].claim + " — is that right?", section: null});

  const seen = new Set(), list = [];
  for(const item of out){
    const k = liveKey(item.q, item.section ? item.section.id : null);
    if(seen.has(k) || live[k]) continue;   // never pay twice for the same answer
    seen.add(k); list.push(item);
  }
  return list;
}
...
function renderFlight(){
  const box = document.getElementById("flightprog");
  const btn = document.getElementById("flightgo");
  if(!flightRun){
    box.style.display = "none"; btn.textContent = "Prepare";
    renderSettings(); return;
  }
  box.style.display = "block"; btn.textContent = "Stop";
  const pct = flightRun.total ? Math.round(100 * flightRun.done / flightRun.total) : 0;
  document.getElementById("flightbar").style.width = pct + "%";
  const left = Math.max(0, flightRun.total - flightRun.done);
  document.getElementById("flightmsg").textContent = flightRun.note
    || (flightRun.done + " of " + flightRun.total + " saved · about "
        + Math.ceil(left * 5.4 / 60) + " min left · you can leave this open and put the phone down");
}

async function prepareForFlight(){
  if(flightRun){ flightRun.stop = true; return; }          // second tap stops it
  if(!canLive()){
    document.getElementById("flightsum").textContent =
      navigator.onLine ? "Needs the live tutor switched on" : "Needs a signal — do this before you fly";
    return;
  }
  const list = flightQuestions();
  if(!list.length){
    document.getElementById("flightsum").textContent = "Already done — nothing left to fetch";
    return;
  }
  flightRun = {stop:false, done:0, total:list.length, failed:0, note:""};
  renderFlight();

  for(const item of list){
    if(flightRun.stop) break;
    try {
      await askLive(item.q, {section: item.section});
      flightRun.done++;
    } catch(e) {
      if(e.kind === "no_free_provider"){
        // Stop rather than hammer an exhausted tier. What was fetched is already saved.
        flightRun.note = "Free models ran out after " + flightRun.done + ". Saved those; try again later.";
        break;
      }
      if(e.kind === "rate_limited"){ await sleep(15000); continue; }   // retry this one
      flightRun.failed++;
      if(flightRun.failed > 8){ flightRun.note = "Too many failures — stopped at " + flightRun.done + "."; break; }
    }
    renderFlight();
    // The server allows twelve a minute. Staying just under it is the difference between this
    // finishing and this spending ten minutes being told to slow down.
    if(!flightRun.stop) await sleep(5400);
  }

  const done = flightRun.done, note = flightRun.note;
  flightRun = null;
  renderFlight();
  document.getElementById("flightsum").textContent =
    note || (done ? done + " answers saved for offline" : "Nothing new to fetch");
}
...
function answer(q, live){
  askSeq++;                             // anything still in flight is now stale
  const box = document.getElementById("answers");
  const hits = search(q);
  const weak = tooWeak(hits, q);
  const written = hits.map(h => h.kind==="entry" ? renderEntry(h.item) : renderSectionAnswer(h.item)).join("");
  const wantLive = liveMode() === "always" || weak;

  // Already asked this once — the saved answer is as good offline as on, so it comes first and
  // costs nothing.
  const cached = liveGet(q, null);
  if(cached && wantLive){
    box.innerHTML = renderLive(cached, q) + alsoWritten(written);
    box.scrollIntoView({behavior:"smooth",block:"start"});
    return;
  }
  if(live && canLive() && wantLive){
    // The closest section goes along as context even when the question came from the Ask tab
    // rather than the reader. "Why is the chapter named after him" needs the chapter.
    const near = hits.find(h => h.kind === "section");
    startThread(q, {section: near ? near.item : null}, alsoWritten(written));
    return;
  }
  if(!weak){
    box.innerHTML = written + (wantLive ? offlineNote(q) : "");
    box.scrollIntoView({behavior:"smooth",block:"start"});
    return;
  }
...
// A section that mentions Gauss is not an answer to who Gauss was, and offline there is nothing
// better to give. Saying so — and saving the question — beats letting the passage stand as
// though it were the reply.
function offlineNote(q){
  if(!usableEndpoint() || liveMode() === "off") return "";
  if(navigator.onLine) return "";
  queueQuestion(q);
  return '<p class="dim" style="margin-top:1rem">That is what the book has on it. If you wanted '
       + 'more than the book says, the tutor can answer properly once you have a signal — '
       + 'saved the question.</p>';
}

// Written explainers sit below a live answer rather than instead of it. They are still the
// better material where they apply — they just are not a reply to the question that was asked.
function alsoWritten(written){
  return written ? '<div class="lbl" style="margin-top:1.6rem">Also written for you</div>' + written : "";
}
...
function missCard(q){
  const has = !!usableEndpoint(), on = liveMode() !== "off";
...
function answerLine(question){
  ...
  // A typed question about a line goes to the tutor, here as much as in the Ask box — this is
  // the strongest case for it, since the section travels with the question. Explain with nothing
  // typed stays offline: that is a lookup, not a question.
  ...
  if(!cached && q && weak && canLive()){
    // The quoted line stays at the top of the thread — it is the context for every turn.
    const spoke = lastWasVoice; lastWasVoice = false;
    startThread(q, {line: line, section: s}, alsoWritten(h.slice(header.length)))
...
// Named for what the reader gets, not for how the code branches.
const LIVE_MODES = [["always","any question"],["gaps","only the gaps"],["off","never"]];
function renderSettings(){
  ...
  if(document.activeElement !== pk) pk.value = provKey();
  document.getElementById("provsum").textContent = provKey()
    ? "Key stored on this device only — never in the app, never in the repo."
    : "Leave blank if the server has its own key. Anything here stays on this device.";
  document.getElementById("livesum").textContent =
    !endpoint()             ? "No address set — offline answers only"
    : !usableEndpoint()     ? "Needs a full https:// address when opened from a file"
    : liveMode() === "off"  ? "Never asked"
    : !navigator.onLine     ? "Waiting for a signal"
    : liveMode() === "always" ? "Ask it anything while you have a signal"
    : "Only when nothing is written on it";
  const n = Object.keys(live).length;
  document.getElementById("cachecount").textContent = n ? n + " kept on this device" : "None yet";
  if(!flightRun){
    const left = canLive() ? flightQuestions().length : null;
    document.getElementById("flightsum").textContent =
      left === null   ? (navigator.onLine ? "Needs the live tutor switched on" : "Do this before you lose signal")
      : left === 0    ? "Done — every section is answered offline"
      : left + " answers to fetch, about " + Math.ceil(left * 5.4 / 60) + " min";
  }
  document.getElementById("flightgo").disabled = !canLive() && !flightRun;
...
document.addEventListener("click", ev => {
  const t = ev.target.closest("[data-v],[data-read],[data-go],[data-lvl],[data-setlvl],[data-settheme],[data-setlive],[data-live],[data-ask],[data-reply],#backidx,#resetprof,#clearcache,#runqueued,#flightgo,#testgo,#replysend");
  if(!t) return;
  ...
  if(t.id==="flightgo"){ prepareForFlight(); return; }
  if(t.id==="testgo"){ testEndpoints(); return; }
  if(t.dataset.reply){ continueThread(t.dataset.reply); return; }
  if(t.id==="replysend"){
    const box = document.getElementById("replybox");
    const v = box && box.value.trim();
    if(v) continueThread(v);
    return;
  }
...
document.getElementById("qbox").addEventListener("input", e => {
  ...
  if(v.trim().length < 3){
    // Emptying the box must also abandon anything still in flight. Otherwise a slow reply lands
    // afterwards and repaints answers to a question that is no longer on screen.
    askSeq++;
...
// Delegated, because the reply box is created and destroyed with every render of the thread.
document.addEventListener("keydown", e => {
  if(e.key !== "Enter" || !e.target || e.target.id !== "replybox") return;
  const v = e.target.value.trim();
  if(v) continueThread(v);
});
document.getElementById("provkey").addEventListener("input", e => setProvKey(e.target.value));
document.getElementById("provkey").addEventListener("change", () => renderSettings());
