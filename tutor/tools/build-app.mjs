...
function usableEndpoint(){
  const ep = endpoint();
  if(!ep) return "";
  const abs = ep.slice(0,8).toLowerCase();
  if(abs.startsWith("http://") || abs.startsWith("https://")) return ep;
  const p = location.protocol;
  return (p === "http:" || p === "https:") ? ep : "";
}
function canLive(){ return !!usableEndpoint() && navigator.onLine && liveMode() !== "off"; }
...
async function askLive(q, ctx){
  ...
  try {
    const res = await fetch(usableEndpoint(), {
...
function liveError(e, q){
  ...
  const unreachable = !e.kind;
  return '<div class="miss"><strong>'
    + (exhausted ? "The free models are all out for now."
                 : esc(e.message || "Could not reach the tutor."))
    + '</strong>'
    + (unreachable ? '<p style="margin:.6rem 0 0">Nothing answered at <span style="font-family:var(--mono);font-size:.8rem">'
        + esc(usableEndpoint()) + '</span>. If that address is wrong, change it in Settings.</p>' : '')
...
document.addEventListener("click", ev => {
  const t = ev.target.closest("[data-v],[data-read],[data-go],[data-lvl],[data-setlvl],[data-settheme],[data-setlive],[data-live],[data-ask],#backidx,#resetprof,#clearcache,#runqueued,#flightgo");
  if(!t) return;
